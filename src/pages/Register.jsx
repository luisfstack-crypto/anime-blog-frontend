...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      // 3. Conexión al endpoint de Registro
      const response = await fetch('https://localhost:7288/api/Auth/register', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ 
          username: username, // Asegúrate que en C# se llame así o "name"
          email: email, 
          password: password 
        }),
      });
      ...
      if (response.ok) {
        ...
        setErrorMsg(errText || 'Error al registrar usuario.');
      }
    } catch (err) {
  ...
  return (
    <>
      <style>{`
        ...
        .reg-form-sub { font-size: 0.78rem; font-weight: 300; color: #4A4240; margin-bottom: 1.5rem; }
